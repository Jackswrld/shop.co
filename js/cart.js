...
const PROMO_CODES = {
  SAVE20:    { type: "percent", value: 20,  label: "-20%" },
  WELCOME10: { type: "percent", value: 10,  label: "-10%" },
  FLAT15:    { type: "fixed",   value: 15,  label: "-$15" },
};
...
function loadCartFromStorage() {
  try {
    ...
    console.warn("Cart data corrupted, resetting:", err);
    return [];
  }
}
...
function renderCart() {
  ...
  itemsList.innerHTML = state.cart
...
function applyPromoCode() {
  ...
  renderOrderSummary();
  showPromoMessage(`✓ "${code}" applied!`, "success");
  input.value = "";
}
